...
function addPageEventListeners()  {
    ...
    var discount_amount = $('#id_bonus_payment').val();
    var modal_loading = $('#modal-discount');
    $('#id_bonus_payment').on('keyup keypress blur change', function() {
        var form = $(this.form);
        if(currentCartTimer) {
            clearTimeout(currentCartTimer);
        }
        currentCartTimer = setTimeout(function () {
            modal_loading.addClass('modal-bg').show();
            var current_discount = $('#id_bonus_payment').val();
            var form_values = form.serialize();
            if(!current_discount) {
                form_values += '&unbind_bonus_payment=1';
            }
            if(discount_amount !== current_discount) {
                discount_amount = current_discount;
                 $.post(form.attr('action'), form_values, function(data) {
                    modal_loading.removeClass('modal-bg').hide();
                    $('.cart-page').replaceWith(data);
                    addPageEventListeners();
                    $('#id_bonus_payment').focus().val('').val(discount_amount);
                });
            } else {
                modal_loading.removeClass('modal-bg').hide();
            }
    ...
    $('#id_promo_code').on('keyup keypress blur change', function() {
        var form = $(this.form);
        if(currentCartTimer) {
            clearTimeout(currentCartTimer);
        }
        currentCartTimer = setTimeout(function () {
            modal_loading.addClass('modal-bg').show();
            var current_code = $('#id_promo_code').val();
            var form_values = form.serialize();
            if(!current_code) {
                form_values += '&unbind_promo_code=1';
            }
            if(promo_code !== current_code) {
                promo_code = current_code;
                 $.post(form.attr('action'), form_values, function(data) {
                    modal_loading.removeClass('modal-bg').hide();
                    $('.cart-page').replaceWith(data);
                    addPageEventListeners();
                    $('#id_promo_code').focus().val('').val(promo_code);
                });
            } else {
                modal_loading.removeClass('modal-bg').hide();
            }
...
jQuery(document).ready(function ($) {
    var dropshippingMark = $('div[dropshipping_mark]').attr('dropshipping_mark');
...
$(document).ready(function () {
    function show_confirmation_phone_time() {
        ...
        var call_immediately = $('#id_call_immediately:checked').length > 0;
        $('#id_confirmation_phone_time')
            .prop('readonly', call_immediately)
            .prop('required', !call_immediately)
            .prop('disabled', call_immediately)
            .val('');
    ...
    $('input[name=confirmation_method]').click(function () {
        ...
        if (this.value != 'phone'){
            $('#id_confirmation_phone_time').prop('required', false)
        }
    });
    $('#id_call_immediately').change(function () {
        show_confirmation_phone_time();
    });
    show_confirmation_phone_time();
});
